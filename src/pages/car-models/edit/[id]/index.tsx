...
import { useRoqClient, useCarModelFindFirst } from 'lib/roq';
...
function CarModelEditPage() {
  ...
  const { data, error, isLoading, mutate } = useCarModelFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: CarModelInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.car_model.update({
        data: values as RoqTypes.car_model,
        where: {
          id,
        },
      });
